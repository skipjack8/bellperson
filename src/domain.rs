...
use crate::{gpu, EngineExt};

use log::{info, warn};

pub struct EvaluationDomain<E: EngineExt, G: Group<E::Fr>> {
    coeffs: Vec<G>,
    exp: u32,
    omega: E::Fr,
    omegainv: E::Fr,
...
impl<E: EngineExt, G: Group<E::Fr>> AsRef<[G]> for EvaluationDomain<E, G> {
    fn as_ref(&self) -> &[G] {
        &self.coeffs
    }
}

impl<E: EngineExt, G: Group<E::Fr>> AsMut<[G]> for EvaluationDomain<E, G> {
    fn as_mut(&mut self) -> &mut [G] {
        &mut self.coeffs
    }
}

impl<E: EngineExt, G: Group<E::Fr>> EvaluationDomain<E, G> {
...
fn best_fft<E: EngineExt, T: Group<E::Fr>>(
    kern: &mut Option<gpu::LockedFFTKernel<E>>,
    a: &mut [T],
    worker: &Worker,
    omega: &E::Fr,
...
pub fn gpu_fft<E: EngineExt, T: Group<E::Fr>>(
...
fn parallel_fft_consistency() {
    ...
    fn test_consistency<E: EngineExt, R: RngCore>(rng: &mut R) {
        let worker = Worker::new();
